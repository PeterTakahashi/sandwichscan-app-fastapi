...
async def fetch_factory_logs_from_bq(
    ...
    sql = build_bq_sql(
        table,
        factory_addr,
        SIG_POOL_CREATED if is_v3 else SIG_PAIR_CREATED,
        from_block,
        to_block,
    )
    client = bq_client()
        ...
        if rows:
            stmt = (
                ...
                .on_conflict_do_update(
                    index_elements=[
                        Token.address
                    ],  # 現行スキーマは address UNIQUE（将来は (chain_id,address) 推奨）
                    set_={
...
async def backfill_all(
    ...
    async with async_session_maker() as session:
        ...
        for (
            ...
            if not logs:
                print(
                    f"[{chain_name}] no logs via BQ for factory {factory_addr} ({ver_name})"
                )
                continue

            token_addrs: Set[str] = set()
            decoded_rows: List[Tuple[str, str, Optional[int], str]] = (
                []
            )  # (t0,t1,fee_or_None,pool_addr)
            ...
            for t0, t1, fee_or_none, pool_addr in decoded_rows:
                ...
                if not (t0_id and t1_id):
                    continue
                swap_fee = (
                    (fee_or_none / 1_000_000) if fee_or_none is not None else 0.003
                )
                pool_rows.append(
                    {
            ...
            print(
                f"[{chain_name}] {ver_name} factory {factory_addr}: pools upserted={inserted}"
            )
